...
impl Fs {
    ...
    fn dump(&mut self) {
        ...
        for i in 0..self.numInodes {
            ...
            if ftype == "free" {
                ...
                print!("[{} a:{} r:{}]",ftype,self.inodes[i as usize].get_addr(),self.inodes[i as usize].get_ref_cnt());
            }
        }
        println!("");
    ...
    fn make_name(&mut self) -> String{
        ...
        let p = ["a","b","c","d","e","f","g","h","j","k","m","n","o","p","q","r","s","t","u","v","w","x","y","z"];
...
pub fn vsfs_op_parse(op_vec:Vec<&str>) {
    let mut vsfs_op = VsfsOption::new();
    let mut i =1;
    while i<op_vec.len() {
        match op_vec[i] {
            "-s" =>{vsfs_op.seed = op_vec[i+1].parse().unwrap();i = i+2;},
            "-i" =>{vsfs_op.numInodes = op_vec[i+1].parse().unwrap();i = i+2;},
            "-d" =>{vsfs_op.numData = op_vec[i+1].parse().unwrap();i = i+2;},
            "-n" =>{vsfs_op.numRequests = op_vec[i+1].parse().unwrap();i=i+2;},
            "-r" =>{vsfs_op.reverse = true;i=i+1;},
            "-p" =>{vsfs_op.printFinal =true;i=i+1;},
            "-c" =>{vsfs_op.solve = true;i=i+1;},
            _ => println!("vsfs_op_parse match wrong!!"),
        }
    }
