...
pub fn segmentation_op_parse(op_vec:Vec<&str>) {
        let mut segm_op = SegmentationOption::new();
        let mut i =1;
        while i<op_vec.len() {
            match op_vec[i] {
                "-s" =>{segm_op.seed = op_vec[i+1].parse().unwrap();i = i+2;},
                "-A" =>{segm_op.addresses = op_vec[i+1].to_string();i = i+2;},
                "-a" =>{segm_op.asize = op_vec[i+1].to_string();i = i+2;},
                "-p" =>{segm_op.psize = op_vec[i+1].to_string();i=i+2;},
                "-n" =>{segm_op.num = op_vec[i+1].parse().unwrap();i=i+2;},
                "-b" =>{segm_op.base0 = op_vec[i+1].to_string();;i=i+2;},
                "-l" =>{segm_op.len0 = op_vec[i+1].to_string();;i=i+2;},
                "-B" =>{segm_op.base1 = op_vec[i+1].to_string();;i=i+2;},
                "-L" =>{segm_op.len1 = op_vec[i+1].to_string();;i=i+2;},
                "-c" =>{segm_op.solve = true;i=i+1;},
                _ => println!("segmentation_op_parse match wrong!!"),
            }
        }
...
fn execute_segmentation_op(options:SegmentationOption) {
    ...
    if len0 > asize / 2 || len1 > asize / 2 {         // not very good idea
    ...
    println! ("");
    println! ("  Segment 0 base  (grows positive) : 0x{:x} (decimal {})",base0, base0);
    println! ("  Segment 0 limit                  : {}" , len0);
    println! ("");
    println! ("  Segment 1 base  (grows negative) : 0x{:x} (decimal {})" ,base1+len1, base1+len1);
    ...
    if addresses == "-1" {
        ...
        for t in vec {
            addrList.push(t.to_string().parse().unwrap());
        }
    }
    ...
    for vStr in &addrList {
        let vaddr:i32 = vStr.to_string().parse().unwrap();
        if vaddr < 0 || vaddr >= asize {
            println!("Error: virtual address {} cannot be generated in an address space of size {}" , vaddr, asize);
            return
        }
        ...
        if options.solve == false {
            ...
            if (vaddr >= (asize/2)) {
                ...
                if paddr < base1{
                    println! ("  VA {}: 0x{:x} (decimal:  {}) --> SEGMENTATION VIOLATION (SEG1)" ,i, vaddr, vaddr);
                }
                else{
                    println! ("  VA {}: 0x{:x} (decimal:  {}) --> VALID in SEG1: 0x{:x} (decimal:  {})" , i, vaddr, vaddr, paddr, paddr);
                }
            }else {
                if vaddr >= len0 {
                    println! ("  VA  {}: 0x{:x} (decimal: {}) --> SEGMENTATION VIOLATION (SEG0)" , i, vaddr, vaddr);
                }else {
                    paddr = vaddr + base0;
                    println! ("  VA  {}: 0x{:x} (decimal:  {}) --> VALID in SEG0: 0x{:x} (decimal:  {})" , i, vaddr, vaddr, paddr, paddr);
                }
            }
        }
        i= i+1;
    ...
    if options.solve == false {
        ...
        println!("this problem, you should assume a simple address space with two segments: the top");
        println!("bit of the virtual address can thus be used to check whether the virtual address");
        println!("is in segment 0 (topbit=0) or segment 1 (topbit=1). Note that the base/limit pairs");
        println!("given to you grow in different directions, depending on the segment, i.e., segment 0");
