...
impl Server {
    ...
    fn get(&mut self, client:String, fname:String) -> i32{
        assert!(self.contents.contains_key(&fname),"server:get() -- file:{} not found on server",fname);
        self.getcnt += 1;
    ...
    fn put(&mut self, client:String, fname:String,value:i32) {
        assert!(self.contents.contains_key(&fname),"server:put() -- file:{} not found on server",fname.clone());
        self.putcnt += 1;
        self.contents.insert(fname.clone(),value);
        if self.solve && isset(self.detail,0) {
            println!("getfile:{} c:{} [{}]" , fname.clone(), client, self.contents[&fname]);
        }
...
impl FileDesc {
    ...
    fn alloc (&mut self,fname:String,sfd:i32) -> i32 {
        if sfd != -1 {
            assert!(self.fd[&sfd] == "" , "filedesc:alloc() -- fd:{} already in use, cannot allocate", sfd);
    ...
    fn lookup(&self,sfd:i32) ->String{
        ...
        assert!(self.fd[&sfd] != "",        "filedesc:lookup() -- fd:{} not in use, cannot lookup" , sfd);
...
impl Cache {
    ...
    fn stats(&self) {
        println!("   Cache -- Hits:{} Misses:{} Invalidates:{}",self.hitcnt, self.misscnt, self.invalidcnt );
    }

    fn put(&mut self,fname:String,data:i32,dirty:bool,refcnt:i32) {
        let cc = CacheContent{data:data,dirty:dirty,refcnt:refcnt,valid:true,};
        self.cache.insert(fname,cc);
    }

    fn update(&mut self,fname:String,data:i32){
        let cc = CacheContent{data:data,dirty:true,refcnt:self.cache[&fname].refcnt,valid:self.cache[&fname].valid,};
        self.cache.insert(fname,cc);
    }

    fn invalidate(&mut self,fname:String) {
        assert!(self.cache.contains_key(&fname),"cache:invalidate() -- cannot invalidate file not in cache ({})",fname);
        self.invalidcnt += 1;
        let cc = CacheContent{data:self.cache[&fname].data,dirty:self.cache[&fname].dirty,refcnt:self.cache[&fname].refcnt,valid:false,};
    ...
    fn checkvalid(&self,fname:String) {
        assert!(self.cache.contains_key(&fname),"cache:checkvalid() -- cannot checkvalid on file not in cache ({})",fname);
    ...
    fn printstate (&self,fname:String) {
        ...
        if self.cache.contains_key(&fname) {   
            ...
            if self.solve && isset(self.detail,2) {
                dospace(self.num);
                if isset(self.detail, 3) {
                    println!("{} [{}:{} (v={},d={},r={})]",self.name, fname, data, validPrint, dirtyPrint, refcnt);
                }else {
                    println!("[{}:{} (v={},d={},r={})]",fname, data, validPrint, dirtyPrint, refcnt);
                }
            }
        }
    }
...
impl Client {
    fn new(name:String,cid:i32,server:Server,files:Files,bias:Vec<f64>,actions:String,solve:bool,detail:i32) -> Client {
        Client{
            name:name.clone(),
            cid:cid,
            server:server,
    ...
    fn init(&mut self,numsteps:i32) {
        ...
        if self.actions == "" {
            for i in 0..numsteps {
                ...
                assert!(fd>=0,"client:init() -- ran out of file descriptors, sorry!");
    ...
    fn stats(&self) {
        println!("{}       -- Reads:{} Writes:{}" , self.name, self.readcnt, self.writecnt);
        self.cache.stats();
    }
...
fn execute_afs_op(options:AfsOption) {
    ...
    assert!(numfiles > 0 && numfiles <= 26, "main: can only simulate 26 or fewer files, sorry");
    assert!(readratio >= 0.0 && readratio <= 1.0, "main: read ratio must be between 0 and 1 inclusive");
    ...
    if actions != "" {
        ...
        for clist in cactions {
            let mut c = Client::new(format!("c{}",i),i,s.clone(),f.clone(),Vec::new(),clist.to_string(),solve,detail);
        ...
        for i in 0..numclients {
            let mut c = Client::new(format!("c{}",i),i,s.clone(),f.clone(),vec![readratio,1.0],"".to_string(),solve,detail);
            c.init(numsteps);
            clients.push(c);
        }
    }
    ...
    if schedule != "" {
        for i in 0..clients.len() {
            ...
            assert!(cnt!=0,"main: client {} not in schedule:{}, which would never terminate",i,schedule);
        }
    }
