...
pub fn lottery_op_parse(op_vec:Vec<&str>) {
    let mut lott_op = LotteryOption::new();
    let mut i =1;
    while i<op_vec.len() {
        match op_vec[i] {
            "-s" =>{lott_op.seed = op_vec[i+1].parse().unwrap();i = i+2;},
            "-j" =>{lott_op.jobs = op_vec[i+1].parse().unwrap();i = i+2;},
            "-l" =>{lott_op.jlist = op_vec[i+1].to_string();i=i+2;},
            "-m" =>{lott_op.maxlen = op_vec[i+1].parse().unwrap();i=i+2;},
            "-p" =>{lott_op.maxticket = op_vec[i+1].parse().unwrap();i=i+2;},
            "-q" =>{lott_op.quantum = op_vec[i+1].parse().unwrap();i=i+2;},
            "-c" =>{lott_op.solve = true;i=i+1;},
            _ => println!("lottery_op_parse match wrong!!"),
        }
    }
...
fn execute_lottery_op(options:LotteryOption) {
    ...
    if options.jlist == "" {
        for jobnum in 0..options.jobs {
            ...
            joblist.push(v);
            println!("  Job {} ( length = {}, tickets = {} )" ,jobnum, runtime, tickets);
        }
    }else {
        let mut jobnum = 0;
        ...
        for job in &joblist {
            println!("  Job {} ( length = {}, tickets = {} )" , job[0], job[1], job[2]);
        }
    }
    println!("");
    ...
    if options.solve == true {
        ...
        for i in 0..runTotal {
            ...
            println!("Random {} -> Winning ticket {} (of {}) -> Run {} ",r ,winner, tickTotal, wjob);
            ...
            for v in &joblist {
                ...
                println!("{} job : {} timeletft: {} tix: {} ",wstr,v[0],v[1],tstr);
            }
            println!("");
